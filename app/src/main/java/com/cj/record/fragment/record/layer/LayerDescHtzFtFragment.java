...
public class LayerDescHtzFtFragment extends RecordBaseFragment {
                                ...
                                public void onNegative(MaterialDialog dialog) {
                                    ...
                                    new MaterialDialog.Builder(mActivity).title(R.string.custom).inputType(InputType.TYPE_CLASS_TEXT |
                                        ...
                                        public void onInput(MaterialDialog dialog, CharSequence input) {
                                            ...
                                            bhwDialog.show();
                                            dictionaryList.add(new Dictionary("1", "黄土状粉土_包含物", input.toString(), bhwList.size() + "", userID, Record.TYPE_LAYER));
                                        }
                                    }).show();
                                }
    ...
    public boolean layerValidator() {
        if (sortNoYs > 0) {
            dictionaryDao.addDictionary(new Dictionary("1", "黄土状粉土_颜色", sprYs.getText().toString(), "" + sortNoYs, userID, Record.TYPE_LAYER));
        }
        if (sortNoKlzc > 0) {
            dictionaryDao.addDictionary(new Dictionary("1", "黄土状粉土_颗粒组成", sprKlzc.getText().toString(), "" + sortNoKlzc, userID, Record.TYPE_LAYER));
        }
        if (sortNoKx > 0) {
            dictionaryDao.addDictionary(new Dictionary("1", "黄土状粉土_孔隙", sprKx.getText().toString(), "" + sortNoKx, userID, Record.TYPE_LAYER));
        }
