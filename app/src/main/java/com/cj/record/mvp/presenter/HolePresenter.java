...
import com.cj.record.mvp.model.HoleModel;
...
public class HolePresenter extends BasePresenter<HoleContract.View> implements HoleContract.Presenter {
    ...
    public void loadData(String projectID, int page, int size, String search, String sort) {
        ...
        model.loadData(projectID, page, size, search, sort)
                .compose(RxScheduler.<PageBean<Hole>>Flo_io_main())
                .as(mView.<PageBean<Hole>>bindAutoDispose())
                .subscribe(new Consumer<PageBean<Hole>>() {
                    @Override
                    public void accept(PageBean<Hole> bean) throws Exception {
                        mView.hideLoading();
                        mView.onSuccessList(bean);
                    }
    ...
    public void delete(Hole hole) {
        ...
        model.delete(hole)
                ...
                .subscribe(new Consumer<Boolean>() {
                    @Override
                    public void accept(Boolean bean) throws Exception {
                        mView.hideLoading();
